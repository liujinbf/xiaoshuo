...
function buildHook(input, profile) {
  ...
  const moment = pick(["周三下午", "深夜十一点", "开会前五分钟", "饭局散场后", "签字的前一晚", "她们以为我睡着的时候"]);
  ...
  const hooks = [
    ...
    `事情发生在${moment}。那时${P}还不知道，“${theme}”只是一个幌子，真正的局从三个月前就已经开始了。`,
    `${P}当时在削苹果，削到一半忽然想起：${secret}。苹果没削完，这件事却再也没办法假装不知道了。`,
    `后来${P}复盘过无数次——如果不是“${theme}”这件事被拿到台面上，${her}或许还会再忍三年。但有些事一旦说破，就再也回不去了。`,
  ];
  return pick(hooks);
}
